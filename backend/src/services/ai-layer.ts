...
function buildAssignmentSupportPrompt(input: AssignmentSupportInput) {
  ...
  const systemPrompt = [
    ...
    "- Use student-provided notes, class points, or placement examples when they are relevant.",
    "- Never invent personal experiences, patient encounters, interviews, or observations the student did not provide.",
    ...
    "- DRAFT mode (UNLOCKED AT 50% UNDERSTANDING): Generate a professional, well-structured assignment draft incorporating the student's provided references with proper citations. The result should be strong enough for student review and personalization.",
    "",
    "PROFESSIONAL DRAFT MODE (50%+ Understanding):",
    "- Generate a complete, professionally written assignment draft the student can review and personalize.",
    ...
    "- Use student-provided notes, class points, or placement examples where relevant, but never invent personal experiences.",
    "- Include a formatted reference list at the end.",
    "- Structure with clear introduction, body paragraphs, and conclusion.",
    "- draftResponse MUST be cleanly formatted for export and on-screen reading.",
    "- Use markdown-style headings so the app can format the assignment properly.",
    "- Preferred structure:",
    "  ## Introduction",
    "  ## Main Discussion",
    "  ### Thematic subheading(s) where useful",
    "  ## Conclusion",
    "  ## Reference List",
    "- Write full academic paragraphs under each heading, not bullet lists, unless the brief explicitly requires lists.",
    "- Do not use bracketed placeholders such as [insert example here] or [student should add].",
    "- Make the wording specific to the assignment brief, programme, and any lecturer feedback provided.",
    "- Address all marking criteria if provided.",
    "- Match the required word count if specified.",
    "",
    "SPECIFICITY AND AUTHENTICITY IN DRAFT MODE:",
    "- Ground the draft in the exact assignment brief, marking criteria, lecturer feedback, and student-provided notes.",
    "- Use student-provided class notes, placement observations, or examples when available.",
    "- If no personal notes are provided, keep examples general and encourage the student to add their own real examples before submission.",
    "- Never fabricate personal reflections, interviews, patient stories, or field observations.",
    "- Prefer precise, concrete wording over generic filler or repetitive stock phrases.",
    "- Every section should feel tailored to this assignment brief, not like a reusable template.",
    citationGuidance,
    wordCountGuidance,
    readinessGuidance,
    referencesGuidance,
  ...
  const userPrompt = [
    ...
    input.personalInsights
      ? `Student-provided notes, examples, or observations:\n${clipAssignmentSupportText(input.personalInsights, 3000)}`
      : "Student-provided notes, examples, or observations: None provided. Do not invent personal experiences; keep examples general unless the student gives them.",
    ...
    input.mode === "draft" && (input.understandingScore ?? 0) >= 50 
      ? [
          ...
          "- Format draftResponse with markdown-style headings such as ## Introduction, ## Main Discussion, ## Conclusion, and ## Reference List.",
          "- Use full paragraphs, not bullet points, unless the brief explicitly asks for list formatting.",
          "- Address ALL marking criteria if provided.",
          "- Include a formatted reference list at the end.",
          `- Target ${input.wordCount ?? 1500} words.`,
          "- Make it strong, specific, and ready for student review before submission.",
          "",
          "- Use any student-provided notes, class points, or placement observations naturally where relevant.",
          "- If no personal notes were provided, keep examples general and leave room for the student to add their own authentic examples during review.",
          "- Never fabricate personal experiences, quotations, interviews, or case details.",
          "- Aim for specific, evidence-based prose rather than generic filler.",
        ].join("\n")
      : [
...
function buildAssignmentSupportFallback(
  ...
  return {
    ...
    draftResponse: [
      ...
      "",
      `## Reference List`,
      `Add your lecturer-required sources and any verified references you actually used when turning this scaffold into your own final submission.`,
    ].join("\n"),
    readyForDraft: true,
    suggestedMode: "draft",
    // Enhanced pedagogy fields
